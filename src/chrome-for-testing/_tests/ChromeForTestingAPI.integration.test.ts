...
describe.skip('ChromeForTestingApi Integration Tests', () => {
  ...
  it('should start download and stop after first few bytes', async () => {
    ...
    const options: DownloadOptions = {
      ...
      timeout: 2000, // 5 second timeout - short to stop download quickly
      onProgress: (progress) => {
        ...
        console.log(
          `Download progress: ${progress.downloaded}/${progress.total} bytes (${progress.percentage.toFixed(1)}%)`,
        )
      },
    }
    ...
    if (result.success) {
      ...
      // biome-ignore lint/nursery/noUnnecessaryConditions: variables can be set asynchronously in onProgress callback
