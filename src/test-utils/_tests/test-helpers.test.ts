...
describe('Test Helpers', () => {
  ...
  describe('element utilities', () => {
    let capturedUtilsPage: Page | null = null

    const elementUtilsPage = () => expectNotNull(capturedUtilsPage)

    beforeEach(async () => {
      const html = createTestHtml('Helper Test Page', true)
      capturedUtilsPage = await createTestPageWithContent(html, 'element-utils-test')
    })

    afterEach(async () => {
      if (capturedUtilsPage) {
        await safeClosePage(capturedUtilsPage)
        capturedUtilsPage = null
      }
    })
  ...
  describe('page utilities', () => {
    ...
    const pageUtilsPage = () => expectNotNull(capturedUtilsPage)

    beforeEach(async () => {
      const html = createTestHtml('Page Utils Test')
      capturedUtilsPage = await createTestPageWithContent(html, 'page-utils-test')
    })
