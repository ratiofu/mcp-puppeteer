...
import { createTransportPair, TestTransport } from '../TestTransport.js'
...
describe('TestTransport', () => {
  ...
  describe('Message routing', () => {
    ...
    it('should prevent sending before start', async () => {
      ...
      await expect(newTransport.send(testMessage)).rejects.toThrow('Transport not started')
    })
  })
  ...
  describe('Resource management', () => {
    ...
    it('should prevent sending messages after close', async () => {
      ...
      const testMessage = createTestMessage({ method: 'test' })
      await expect(clientTransport.send(testMessage)).rejects.toThrow('Transport is closed')
    })
    ...
    it('should handle cleanup when transport is closed', async () => {
      ...
      const testMessage = createTestMessage({ method: 'test' })
      await expect(serverTransport.send(testMessage)).rejects.toThrow(
        'Transport not connected to another transport',
      )
  ...
  describe('Error handling', () => {
    it('should throw error when sending without connection', async () => {
      ...
      const testMessage = createTestMessage({ method: 'test' })
      await expect(isolatedTransport.send(testMessage)).rejects.toThrow(
        'Transport not connected to another transport',
      )
    })
