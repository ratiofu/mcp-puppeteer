...
describe('Test Context Factory', () => {
  ...
  describe('withTestContext', () => {
    it('should run test with test label', async () => {
      ...
      // After withTestContext completes, the context should be cleaned up
      expect(contextFromTest).not.toBeNull();
      expect(contextFromTest!.sessionId).toMatch(/^basic-test-\d+-[a-z0-9]+$/);
    });
    ...
    it('should run test with config object', async () => {
      ...
      expect(contextFromTest).not.toBeNull();
      expect(contextFromTest!.sessionId).toMatch(/^config-test-\d+-[a-z0-9]+$/);
    });
