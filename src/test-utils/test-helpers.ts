...
/**
 * Ensures a value is not null and returns it, if so
 * @param value The value to guarantee to not be null
 * @returns T
 */
export function expectNotNull<T>(value: T | null): NonNullable<T> {
 ...
 */
export async function takeScreenshotBase64(
  page: Page,
 ...
 */
export async function executeScript<T extends {}>(page: Page, script: string): Promise<T> {
 ...
 */
export function createTestHtml(title = 'Test Page', includeConsoleScript = false): string {
