...
import { TOOL_NAMES, type GetHtmlResponse } from '../types/api.js';
  ...
  describe('Valid HTML Content Extraction', () => {
    it('should extract HTML content from test pages', async () => {
      await withTestContext('html-basic-extraction', async (context) => {
        ...
        const result = await context.client.callTool(TOOL_NAMES.GET_HTML, {}) as GetHtmlResponse;
    ...
    it('should extract complete HTML content including dynamic elements', async () => {
      await withTestContext('html-dynamic-content', async (context) => {
        ...
        expect(htmlContent).toContain('<div id="result" class="result">Button clicked 1 times</div>');
      });
    });
    ...
    it('should extract HTML from different page types', async () => {
      await withTestContext('html-different-pages', async (context) => {
        ...
        const result1 = await context.client.callTool(TOOL_NAMES.GET_HTML, {}) as GetHtmlResponse;
        ...
        const result2 = await context.client.callTool(TOOL_NAMES.GET_HTML, {}) as GetHtmlResponse;
  ...
  describe('Error Handling', () => {
    it('should handle error when no page is available', async () => {
      await withTestContext('html-no-page', async (context) => {
        ...
        const content = result.content[0];
        expect(content.type).toBe('text');
        expect(content.text).toBe('no current page to extract HTML from');
      });
    });
...
<body>
    ...
    <script>
        ...
        const beforeResult = await context.client.callTool(TOOL_NAMES.GET_HTML, {}) as GetHtmlResponse;
        ...
        const afterResult = await context.client.callTool(TOOL_NAMES.GET_HTML, {}) as GetHtmlResponse;
