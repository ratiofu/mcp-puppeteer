...
import { initBrowserSafe } from '../initBrowser.js'
import { withTestContext } from '../test-utils/TestContext.js'
import type { BrowserConnectionConfig } from '../types/internal.js'
...
describe('Integration Tests - Real Component Integration', () => {
  ...
  afterEach(async () => {
    ...
    if (browser) {
      try {
        ...
        // Brow_erroright already be closed
      }
      browser = null
    }
  })
  ...
  describe('Browser Connection Integration', () => {
    ...
    it('should handle custom configuration with initBrowserSafe', async () => {
      const customConfig: BrowserConnectionConfig = {
        // biome-ignore lint/style/useNamingConvention: that's what Chrome expects
