  ...
  constructor(sessionId: string, browser: Browser) {
        ...
        try {
          ...
          return {
            content: [
              {
                type: 'text',
                text: `${ERROR_MESSAGES.navigationFailed}: ${errorToString(error)}`,
              },
            ],
            isError: true,
          }
    ...
    this.tool(
      ...
      async ({ selector }: ClickRequest): Promise<ClickResponse> => {
        if (!this.page) {
          return {
            content: [{ type: 'text', text: ERROR_MESSAGES.noPage }],
            isError: true,
          }
        }
        try {
          ...
          return {
            content: [
              { type: 'text', text: `${ERROR_MESSAGES.clickFailed}: ${errorToString(error)}` },
            ],
            isError: true,
          }
        }
        ...
        try {
          ...
          return {
            content: [
              {
                type: 'text',
                text: `${ERROR_MESSAGES.screenshotFailed}: ${errorToString(error)}`,
              },
            ],
            isError: true,
          }
        ...
        try {
          ...
          return {
            content: [
              {
                type: 'text',
                text: `${ERROR_MESSAGES.htmlExtractionFailed}: ${errorToString(error)}`,
              },
            ],
            isError: true,
          }
    ...
    this.tool(
      ...
      'Get the current console output',
      {
        clear: z
          .boolean()
          .describe('Whether to clear the console after getting the output')
          .default(false),
      },
      async ({ clear }: GetConsoleRequest): Promise<GetConsoleResponse> => {
        ...
        try {
          ...
          return {
            content: [
              { type: 'text', text: output.length > 0 ? output : 'No console output available' },
            ],
            isError: false,
          }
        } catch (error) {
          return {
            content: [
              {
                type: 'text',
                text: `${ERROR_MESSAGES.consoleRetrievalFailed}: ${errorToString(error)}`,
              },
            ],
            isError: true,
          }
  ...
  async disconnect() {
    ...
    if (pageRef) {
      ...
      try {
        ...
        pageRef.close().catch((error) => {
          const msg = errorToString(error)
          // Suppress noisy log when the page was already closed by context
          if (
            !(msg.includes('No target with given id found') || msg.includes('Connection closed'))
          ) {
