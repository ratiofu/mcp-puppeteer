...
import { ChromeForTestingAPI } from '../chrome-for-testing/ChromeForTestingAPI.js';
import { BrowserInstallation } from './BrowserInstallation.js';
import type { BrowserVersion, DownloadProgress } from '../chrome-for-testing/types.js';
import { errorToString } from '../utils/error.js';
import { ConcreteFileSystemOperations, ConcreteProcessOperations, type FileSystemOperations, type ProcessOperations } from '../io/index.js';
...
export interface ChromeForTestingAPIInterface {
  ...
  downloadChromium(version: BrowserVersion, options: any): Promise<any>;
}
...
export interface BrowserInstallationFactory {
  create(path: string, version: string, source: 'system' | 'managed', verified?: boolean): BrowserInstallation;
}
...
export class DefaultBrowserInstallationFactory implements BrowserInstallationFactory {
  create(path: string, version: string, source: 'system' | 'managed', verified: boolean = false): BrowserInstallation {
...
export class BrowserManagerService {
  ...
  constructor(
    ...
    chromeForTestingAPI?: ChromeForTestingAPIInterface,
    fs?: FileSystemOperations,
    process?: ProcessOperations,
    browserFactory?: BrowserInstallationFactory
  ) {
    // Default managed installation path: ~/.puppeteer-mcp/chromium/
    this.managedInstallPath = managedInstallPath || join(homedir(), '.puppeteer-mcp', 'chromium');
    this.chromeForTestingAPI = chromeForTestingAPI || new ChromeForTestingAPI();
  ...
  async installChromium(request: InstallChromiumManagerRequest = {}): Promise<InstallationResult> {
    try {
      ...
      const installation = this.browserFactory.create(executablePath, versionToInstall.version, 'managed', false);
   ...
   */
  async cleanupOldVersions(keepCount: number = 2): Promise<CleanupResponse> {
    ...
    try {
      ...
      const sortedVersions = versionDirs.sort((a, b) => this.compareVersions(b, a));
  ...
  async createInstallation(path: string): Promise<BrowserInstallation> {
    ...
    if (version === 'unknown') {
      try {
        ...
        const output = this.process.execSync(command, { encoding: 'utf8', stdio: 'pipe' } as ExecSyncOptions).trim();
   ...
   * @private
   */
  private async getExistingInstallation(version: string): Promise<BrowserInstallation | null> {
    ...
    const executablePath = await this.findExecutableInDirectory(versionPath);
    if (!executablePath) {
      return null;
    }
  ...
  private async findExecutableInExtraction(extractedPath: string): Promise<string | null> {
    ...
    return await this.findExecutableRecursively(extractedPath, executableNames);
  }
   ...
   * @private
   */
  private async findExecutableRecursively(directory: string, executableNames: string[]): Promise<string | null> {
    try {
      ...
      for (const entry of entries) {
        if (entry.isDirectory()) {
          ...
          const found = await this.findExecutableRecursively(subdirPath, executableNames);
          if (found) {
            return found;
          }
        }
   ...
   * @private
   */
  private async findExecutableInDirectory(directory: string): Promise<string | null> {
    const executableNames = process.platform === 'win32'
      ? ['chrome.exe', 'chromium.exe']
      : ['chrome', 'chromium'];
